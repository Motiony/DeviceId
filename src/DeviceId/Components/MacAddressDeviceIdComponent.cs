...
public class MacAddressDeviceIdComponent : IDeviceIdComponent
{
    ...
    /// <param name="excludeDockerBridge">A value determining whether docker devices should be excluded.</param>
    public MacAddressDeviceIdComponent(bool excludeWireless, bool excludeDockerBridge = true)
    {
    ...
    public string GetValue()
    {
        var values = NetworkInterface.GetAllNetworkInterfaces()
            .Where(x => (!_excludeWireless || (x.NetworkInterfaceType != NetworkInterfaceType.Wireless80211 && (!OS.IsLinux || x.Description.StartsWith("en")))) && (!_excludeDockerBridge || x.Name != _dockerBridgeInterfaceName))
